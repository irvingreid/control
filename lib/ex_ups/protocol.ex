...
defmodule ExUps.Parsers do
  ...
  begin_list_ups = replace(string("BEGIN LIST"), :begin) |> replace(string(" UPS"), :ups) |> ignore(newline)
  begin_list_vars = replace(string("BEGIN LIST"), :begin) |> replace(string(" VAR "), :var) |> concat(name) |> ignore(newline)

  end_list_vars = replace(string("END LIST"), :end) |> replace(string(" VAR "), :var) |> concat(name) |> ignore(newline)
